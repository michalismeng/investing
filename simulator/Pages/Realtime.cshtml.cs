...
public class RealtimeModel : PageModel
{
    ...
    public void OnGet(string ticker = "AMZN", string? cutoff = null, int years=5)
    {
        ...
        Volume_MA_50_day = Records.Select(r => new TickerData() { Date = r.Date, Ticker = r.Ticker, Close = r.Volume })
                                  .GetSma(50)
                                  .Condense()
                                  .ToList();
    }
    ...
    public IActionResult OnGetPrice(string ticker, DateTime date)
    {
        ...
        return new OkObjectResult(new {
            Price = price.Last(),
            VolumeMA50day = price.Count >= 50 ? price.Select(r => new TickerData() { Date = r.Date, Ticker = r.Ticker, Close = r.Volume }).GetSma(50).Condense().ToList().Last() : new SmaResult(price.Last().Date),
